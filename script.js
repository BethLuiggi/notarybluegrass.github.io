...
$('calcMilesBtn').addEventListener('click',function(){
  ...
  // Nominatim's structured query is much more accurate than free-form text
  var nomURL='https://nominatim.openstreetmap.org/search?format=json&addressdetails=1&limit=1&countrycodes=us'
  ...
  .then(function(data){
    if(!data||!data[0]) throw new Error('not_found');
    var d=data[0]; var details=d.address||{};
    if(!details.house_number || !details.road) throw new Error('not_specific');

    // Verify state matches what user picked (Nominatim returns full name; we compare case-insensitive)
    var stateMap={};
    for(var i=0;i<US_STATES.length;i++){ stateMap[US_STATES[i][1].toLowerCase()]=US_STATES[i][0]; }
    var returned=(details.state||'').toLowerCase();
    var returnedAbbrev=stateMap[returned]||returned.toUpperCase();
    if(returnedAbbrev!==state.toUpperCase()) throw new Error('state_mismatch');

    var destLat=parseFloat(d.lat), destLon=parseFloat(d.lon);
    res.innerHTML='<span style="color:var(--gray);font-size:.82rem;">Calculating road distance...</span>';
    return fetch('https://router.project-osrm.org/route/v1/driving/'+HOME_LON+','+HOME_LAT+';'+destLon+','+destLat+'?overview=false')
      .then(function(r){return r.json();})
      .then(function(rd){
      ...
      .catch(function(err){
        ...
        var straight=haversineMi(HOME_LAT,HOME_LON,destLat,destLon);
  ...
  .catch(function(err){
    ...
    if(err.message==='not_found' || err.message==='not_specific'){
      showError('&#9888; That address didn\'t match a real US location. Please double-check the street number, street name, city, state, and ZIP. For hospitals, jails, or other non-standard locations, call <a href="tel:2703080260" style="color:var(--gold);text-decoration:underline;">(270) 308-0260</a>.');
    } else if(err.message==='state_mismatch'){
      showError('&#9888; The state you selected doesn\'t match the city/ZIP. Please double-check the state, city, and ZIP combination.');
    } else {
      showError('&#9888; Could not verify that address right now. Please double-check your entries, or call <a href="tel:2703080260" style="color:var(--gold);text-decoration:underline;">(270) 308-0260</a> to book by phone.');
    }
  });
});
