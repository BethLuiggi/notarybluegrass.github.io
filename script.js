...
function buildInvoice(id){
  ...
  if(bk.ahFee>0)lines.push([(bk.ahFee===35?'Holiday fee':'After-hours / Sunday fee'),'+$'+bk.ahFee]);
...
function openBooking(agent){
  bk={agent:agent,notaryPrints:null,pkgFee:0,pkgLabel:'',extraFee:0,signers:2,sigFee:0,witFee:0,ahFee:0,milesFee:0,miles:0,prefDate:'',prefTime:'',timeDisplay:'',scheduledTime:'',isTitleCo:false,needsLocation:false,milesCalc:false};
...
function checkDateTime(){
  ...
  bk.ahFee=isHol?35:((isAH||isSun)?25:0); bk.prefDate=d; bk.prefTime=t;
  ...
  else if(isSun) tags+=' <span style="background:#fff3cd;color:#856404;padding:.15rem .45rem;border-radius:4px;font-size:.78rem;">Sunday +$25</span>';
...
$('payBtn').addEventListener('click',async function(){
  ...
  var bd={
    _subject:'NEW PAID BOOKING - NotaryBluegrass.com',
    _replyto:agentEmail,
    agent:bk.agent,
...
function handleCalendlyMessage(e){
  ...
  if(data.event==='calendly.event_scheduled'){
    ...
    var sigCounty=bk.sigCounty||$('sigCounty').value.trim();
    var resv={
      _subject:'TIME RESERVED - NotaryBluegrass.com',
      _replyto:agentEmail,
      agent_email:agentEmail,
