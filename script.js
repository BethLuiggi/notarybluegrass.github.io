...
$('calcMilesBtn').addEventListener('click',function(){
  ...
  function showError(msg){
    ...
    btn.disabled=false; btn.textContent=addr?'Recalculate':'Calculate Travel Fee';
  }

  if(!addr){alert('Please enter the signing address first.');return;}

  // Pre-validation: must have a street number (digit), letters, and reasonable length
  if(addr.length<8 || !/\d/.test(addr) || !/[a-zA-Z]{3,}/.test(addr)){
    showError('&#9888; Please enter a complete street address with a number (e.g., <em>123 Main St, Elizabethtown, KY 42701</em>).');
    return;
  }
  ...
  fetch('https://nominatim.openstreetmap.org/search?format=json&addressdetails=1&q='+encodeURIComponent(addr)+'&limit=1&countrycodes=us')
  .then(function(r){return r.json();})
  .then(function(data){
    if(!data||!data[0])throw new Error('not_found');
    var d=data[0]; var details=d.address||{};
    // Require a real street address: has both a house number and a road
    ...
    return fetch('https://router.project-osrm.org/route/v1/driving/'+HOME_LON+','+HOME_LAT+';'+destLon+','+destLat+'?overview=false')
      ...
      .catch(function(err){
        console.warn('Road-routing API unavailable; using straight-line × 1.25 estimate. Reason:',err);
  ...
  .catch(function(err){
    ...
    if(err.message==='not_found' || err.message==='not_specific'){
      showError('&#9888; That doesn\'t look like a complete street address. Please include a house number, street name, city, and state (e.g., <em>123 Main St, Elizabethtown, KY 42701</em>). For non-standard locations like hospitals or care facilities, call <a href="tel:2703080260" style="color:var(--gold);text-decoration:underline;">(270) 308-0260</a>.');
    } else {
      showError('&#9888; Could not verify that address right now. Please double-check the spelling and try again, or call <a href="tel:2703080260" style="color:var(--gold);text-decoration:underline;">(270) 308-0260</a> to book by phone.');
    }
  });
});
