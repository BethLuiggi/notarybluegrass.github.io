// Cloudflare Worker endpoint that creates Stripe Checkout sessions.
// Replace YOUR-SUBDOMAIN below with the actual Worker URL after deploy.
var WORKER_URL='https://bluegrass-notary-pay.luiggibgnotary.workers.dev';
var PROMO_CODES={'TITLEME':true,'TITLECO':true};
var TITLE_FEE_TIER1=['Hardin'];
var TITLE_FEE_TIER2=['Jefferson','Bullitt','Nelson','LaRue','Hart','Grayson','Breckinridge','Meade'];
var TITLE_FEE_TIER3=['Hancock','Daviess','Ohio','Butler','Green','Taylor','Marion','Washington','Spencer'];
function calcTitleFee(promo,locType,county){
  if(locType==='in-office') return 75;
  if(locType!=='mobile'||!county) return null;
  if(TITLE_FEE_TIER1.indexOf(county)>=0) return 125;
  if(TITLE_FEE_TIER2.indexOf(county)>=0) return 150;
  if(TITLE_FEE_TIER3.indexOf(county)>=0) return promo==='TITLECO'?200:175;
  return null;
}
...
(function populateStates(){
  var sel=document.getElementById('sigState'); if(!sel) return;
  var html='<option value="">State</option>';
  for(var i=0;i<US_STATES.length;i++){
    var sel2=US_STATES[i][0]==='KY'?' selected':'';
    html+='<option value="'+US_STATES[i][0]+'"'+sel2+'>'+US_STATES[i][1]+'</option>';
  }
  sel.innerHTML=html;
})();
var TOTAL_STEPS=8;
...
var bk={
  ...
  titleFlow:false,promoChoice:null,promoCode:'',
  tLocType:null,tCounty:'',tFee:0,tOfficeName:'',
  tSigners:'',tAddr:'',tContact:''
};
...
function buildInvoice(id){
  ...
  h+='<strong>Location:</strong> '+(typeof getSigAddr==='function'?getSigAddr():'')+(bk.sigCounty?', '+bk.sigCounty:'');
...
function openBooking(agent){
  bk={agent:agent,notaryPrints:null,pkgFee:0,pkgLabel:'',extraFee:0,signers:2,sigFee:0,witFee:0,ahFee:0,milesFee:0,miles:0,prefDate:'',prefTime:'',timeDisplay:'',scheduledTime:'',isTitleCo:false,needsLocation:false,milesCalc:false,titleFlow:false,promoChoice:null,promoCode:'',tLocType:null,tCounty:'',tFee:0,tOfficeName:'',tSigners:'',tAddr:'',tContact:''};
  ...
  if($('tCounty')) $('tCounty').value='';
  if($('tOfficeName')) $('tOfficeName').value='';
  var roOff=$('ro-loc-office'), roMob=$('ro-loc-mobile');
  if(roOff){ roOff.classList.remove('sel'); roOff.querySelector('.ro-dot').style.background=''; roOff.querySelector('.ro-dot').style.borderColor='#ddd'; }
  if(roMob){ roMob.classList.remove('sel'); roMob.querySelector('.ro-dot').style.background=''; roMob.querySelector('.ro-dot').style.borderColor='#ddd'; }
  if($('tOfficeFields')) $('tOfficeFields').style.display='none';
  if($('tMobileFields')) $('tMobileFields').style.display='none';
  if($('tFeePreview')) $('tFeePreview').style.display='none';
  $('tCalEmbed').innerHTML='';
  // Reset step 1
  ...
  $('sigStreetNum').value=''; $('sigStreetName').value=''; $('sigCity').value='';
  if($('sigState')) $('sigState').value='KY';
  $('sigZip').value=''; $('sigCounty').value=''; $('mileageResult').innerHTML='';
  ...
  // Reset step 7
  $('promoMsg').textContent=''; $('promoCode').value=''; $('promoField').style.display='none';
...
$('sPromoInput').addEventListener('input',function(){
  ...
  if(PROMO_CODES[v]){ msg.style.color='var(--green)'; msg.textContent='✓ Code accepted.'; }
...
$('sPromoNext').addEventListener('click',function(){
  if(bk.promoChoice==='yes'){
    ...
    if(!PROMO_CODES[v]){ alert('Please enter a valid promo code, or select "No — Standard booking".'); return; }
    bk.titleFlow=true; bk.isTitleCo=true; bk.promoCode=v;
    var _now=new Date();
...
function updateTitleFeePreview(){
  var fee=calcTitleFee(bk.promoCode, bk.tLocType, bk.tCounty);
  var box=$('tFeePreview');
  if(fee){
    box.style.display='block';
    var label=bk.tLocType==='in-office'?'In-office closing (Hardin County)':'Mobile closing — '+bk.tCounty+' County';
    box.innerHTML='Pre-negotiated fee: <span style="color:var(--green);font-size:1.15rem;">$'+fee+'</span><br/><span style="font-weight:400;font-size:.82rem;color:var(--gray);">'+label+'</span>';
    bk.tFee=fee;
  } else {
...
$('ro-loc-office').addEventListener('click',function(){
  bk.tLocType='in-office'; bk.tCounty='Hardin';
  selectRo('ro-loc-office','ro-loc-mobile',null);
  $('tOfficeFields').style.display='block';
  $('tMobileFields').style.display='none';
  updateTitleFeePreview();
});
$('ro-loc-mobile').addEventListener('click',function(){
  bk.tLocType='mobile';
  selectRo('ro-loc-mobile','ro-loc-office',null);
  $('tOfficeFields').style.display='none';
  $('tMobileFields').style.display='block';
  bk.tCounty=$('tCounty').value;
  updateTitleFeePreview();
});
$('tCounty').addEventListener('change',function(){
  bk.tCounty=this.value;
  updateTitleFeePreview();
});

$('tNext').addEventListener('click',function(){
  if(!bk.tLocType){ alert('Please choose where the signing will take place (in your office or mobile).'); return; }
  if(bk.tLocType==='mobile'){
    if(!$('tAddr').value.trim()){ alert('Please enter the signing address.'); return; }
    if(!$('tCounty').value.trim()){ alert('Please select the Kentucky county.'); return; }
    bk.tCounty=$('tCounty').value.trim();
    bk.tAddr=$('tAddr').value.trim();
  } else {
    bk.tAddr=''; bk.tCounty='Hardin';
    bk.tOfficeName=$('tOfficeName').value.trim();
  }
  ...
  var fee=calcTitleFee(bk.promoCode, bk.tLocType, bk.tCounty);
  if(!fee){ alert('Could not determine the fee for this location. Please re-check your selections or call (270) 308-0260.'); return; }
  bk.tFee=fee;
...
function loadTitleCal(){
  ...
  var locDisplay=bk.tLocType==='in-office'?('In-office (Hardin County)'+(bk.tOfficeName?' — '+bk.tOfficeName:'')):(bk.tAddr+(bk.tCounty?', '+bk.tCounty+' County':''));
  function buildTUrl(agent){
    var u=CAL[agent]+'?hide_gdpr_banner=1&primary_color=c9a84c&name='+nameParam
      ...
      +'&a1='+encodeURIComponent(locDisplay)
      +'&a2='+encodeURIComponent('TITLE CO ['+bk.promoCode+'] $'+bk.tFee+' — Signers: '+signersInline+' | Contact: '+contact);
...
// Helper: build a combined address string from the 5 structured fields
function getSigAddr(){
  var n=$('sigStreetNum').value.trim();
  var s=$('sigStreetName').value.trim();
  var c=$('sigCity').value.trim();
  var st=$('sigState').value.trim();
  var z=$('sigZip').value.trim();
  if(!n||!s||!c||!st||!z) return '';
  return n+' '+s+', '+c+', '+st+' '+z;
}

// Mileage — uses structured Nominatim query for more reliable validation
$('calcMilesBtn').addEventListener('click',function(){
  var streetNum=$('sigStreetNum').value.trim();
  var streetName=$('sigStreetName').value.trim();
  var city=$('sigCity').value.trim();
  var state=$('sigState').value.trim();
  var zip=$('sigZip').value.trim();
  var btn=$('calcMilesBtn'), res=$('mileageResult');

  function showError(msg){
    bk.milesCalc=false; bk.miles=0; bk.milesFee=0;
    res.innerHTML='<span style="color:#e53e3e;font-size:.85rem;font-weight:700;line-height:1.5;display:block;">'+msg+'</span>';
    btn.disabled=false; btn.textContent='Recalculate';
  }

  if(!streetNum) return showError('&#9888; Please enter a street number.');
  if(!/^\d/.test(streetNum)) return showError('&#9888; Street number must start with a digit (e.g., 123 or 123A).');
  if(!streetName) return showError('&#9888; Please enter a street name.');
  if(!city) return showError('&#9888; Please enter a city.');
  if(!state) return showError('&#9888; Please select a state.');
  if(!zip) return showError('&#9888; Please enter a ZIP code.');
  if(!/^\d{5}(-\d{4})?$/.test(zip)) return showError('&#9888; ZIP code must be 5 digits (e.g., 42701) or ZIP+4 (e.g., 42701-1234).');

  btn.disabled=true; btn.textContent='Calculating...';
  res.innerHTML='<span style="color:var(--gray);font-size:.82rem;">Verifying address...</span>';

  // 3040 Dolphin Dr, Elizabethtown, KY 42701
  var HOME_LAT=37.703224, HOME_LON=-85.871674;
  ...
  // No distance cap — we travel anywhere in Kentucky. First 20 miles are
  // included; every mile past 20 bills at $2/mile, however far that is.
  function render(mi, isApprox){
    bk.miles=mi; bk.milesCalc=true;
    var tag=isApprox?' <span style="color:var(--gray);font-weight:400;">(estimated)</span>':'';
    if(mi<=20){
      bk.milesFee=0;
      res.innerHTML='<span style="color:var(--green);font-size:.85rem;font-weight:700;">&#x2705; '+mi+' miles'+tag+' &mdash; travel included!</span>';
    } else {
      var ex=mi-20;
      bk.milesFee=ex*2;
      res.innerHTML='<span style="color:#c9a84c;font-size:.85rem;font-weight:700;">&#128205; '+mi+' miles'+tag+' &mdash; '+ex+' beyond 20 = <strong>+$'+bk.milesFee+' travel fee</strong></span>';
    }
    btn.disabled=false; btn.textContent='Recalculate';
  }

  // Cascade through 3 geocoding strategies — accept the first one that returns coordinates.
  // OSM's free geocoder doesn't have every house number indexed (especially in smaller cities),
  // so we fall back from "exact address" → "free-form" → "city/zip only".
  function geocode(url){
    return fetch(url).then(function(r){return r.json();}).then(function(data){
      if(!data||!data[0]||!data[0].lat||!data[0].lon) throw new Error('no_match');
      return {lat:parseFloat(data[0].lat), lon:parseFloat(data[0].lon)};
    });
  }
  var nomBase='https://nominatim.openstreetmap.org/search?format=json&limit=1&countrycodes=us';
  var structuredURL=nomBase
    +'&street='+encodeURIComponent(streetNum+' '+streetName)
    +'&city='+encodeURIComponent(city)
    +'&state='+encodeURIComponent(state)
    +'&postalcode='+encodeURIComponent(zip5);
  var freeformURL=nomBase+'&q='+encodeURIComponent(streetNum+' '+streetName+', '+city+', '+state+' '+zip5);
  var cityZipURL=nomBase+'&city='+encodeURIComponent(city)+'&state='+encodeURIComponent(state)+'&postalcode='+encodeURIComponent(zip5);

  geocode(structuredURL)
    .catch(function(){ console.info('Structured geocode missed, trying free-form...'); return geocode(freeformURL); })
    .catch(function(){ console.info('Free-form missed, trying city/zip only...'); return geocode(cityZipURL); })
    .then(function(loc){
      res.innerHTML='<span style="color:var(--gray);font-size:.82rem;">Calculating road distance...</span>';
      return fetch('https://router.project-osrm.org/route/v1/driving/'+HOME_LON+','+HOME_LAT+';'+loc.lon+','+loc.lat+'?overview=false')
        .then(function(r){return r.json();})
        .then(function(rd){
          if(!rd||!rd.routes||!rd.routes[0]) throw new Error('no_route');
          return {mi:Math.round(rd.routes[0].distance/1609.34), approx:false};
        })
        .catch(function(err){
          console.warn('Road-routing API unavailable; using straight-line x 1.25 estimate. Reason:',err);
          var straight=haversineMi(HOME_LAT,HOME_LON,loc.lat,loc.lon);
          return {mi:Math.round(straight*1.25), approx:true};
        });
    })
    .then(function(r){ render(r.mi, r.approx); })
    .catch(function(err){
      console.error('Mileage calculation failed at all 3 geocode tiers:',err);
      showError('&#9888; We couldn\'t verify that location. Please double-check your city, state, and ZIP. If the address is a hospital, jail, care facility, or other non-standard place, call <a href="tel:2703080260" style="color:var(--gold);text-decoration:underline;">(270) 308-0260</a> and we\'ll book it for you.');
    });
});
...
$('s5next').addEventListener('click',function(){
  ...
  if(!$('sigStreetNum').value.trim()||!$('sigStreetName').value.trim()||!$('sigCity').value.trim()||!$('sigState').value.trim()||!$('sigZip').value.trim()){alert('Please fill in all signing address fields (street number, street name, city, state, and ZIP).');return;}
...
// (Stripe Elements loading removed — Stripe Checkout now handles card entry on a hosted page.)
...
$('promoCode').addEventListener('input',function(){
  ...
  if(PROMO_CODES[v]){
    bk.isTitleCo=true; bk.promoCode=v; msg.style.color='var(--green)';
    msg.textContent='✓ Title company code accepted. Invoice will be emailed.';
    $('payBtn').textContent='Confirm Booking & Send Invoice';
  } else if(v.length>0){
    bk.isTitleCo=false; bk.promoCode=''; msg.style.color='#e53e3e'; msg.textContent='Invalid code.';
    $('payBtn').textContent='Pay & Confirm Appointment →';
  } else {
    bk.isTitleCo=false; bk.promoCode=''; msg.textContent='';
    $('payBtn').textContent='Pay & Confirm Appointment →';
  }
});
...
$('payBtn').addEventListener('click',async function(){
  ...
  var bd={
    ...
    signing_street_num:$('sigStreetNum').value.trim(),
    signing_street_name:$('sigStreetName').value.trim(),
    signing_city:$('sigCity').value.trim(),
    signing_state:$('sigState').value.trim(),
    signing_zip:$('sigZip').value.trim(),
  ...
  if(bk.isTitleCo){
    ...
    bd._subject='TITLE COMPANY INVOICE ['+(bk.promoCode||'CODE')+'] - Bluegrass-Notary.com';
    bd.type='TITLE_COMPANY_INVOICE';
    bd.promo_code=bk.promoCode||'';
    try{await fetch('https://formspree.io/f/xqenkybe',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(bd)});}catch(e){}
    btn.disabled=false;
    showSuccess(); return;
  }
  // Standard (paying) flow: redirect to Stripe Checkout. Money is captured by Stripe BEFORE
  // Formspree is notified — the Worker's webhook handler relays to Formspree only after
  // Stripe confirms a successful payment.
  btn.disabled=true; btn.textContent='Redirecting to Stripe…';
  try{
    var amount_cents=Math.round(calcTotal()*100);
    var resp=await fetch(WORKER_URL+'/create-checkout-session',{
      method:'POST',
      headers:{'Content-Type':'application/json'},
      body:JSON.stringify({amount_cents:amount_cents, booking_data:bd})
    });
    ...
    var errBox=$('card-errors');
    if(errBox){ errBox.textContent='Payment could not be initiated: '+err.message+'. Please try again or call (270) 308-0260.'; }
    else { alert('Payment could not be initiated: '+err.message); }
  }
});
...
function handleCalendlyMessage(e){
  ...
  if(data.event==='calendly.event_scheduled'){
    ...
    if(bk.titleFlow && curStep==='TitleCal'){
      ...
      var locDesc=bk.tLocType==='in-office'?('In-office (Hardin County)'+(bk.tOfficeName?' — '+bk.tOfficeName:'')):(bk.tAddr+(bk.tCounty?', '+bk.tCounty+' County':''));
      var tbd={
        _subject:'TITLE COMPANY BOOKING ['+bk.promoCode+'] $'+bk.tFee+' - Bluegrass-Notary.com',
        _replyto:titleAgentEmail,
        agent:bk.agent,
        agent_email:titleAgentEmail,
        notify_also:titleAgentEmail,
        type:'TITLE_COMPANY_BOOKING',
        promo_code:bk.promoCode,
        location_type:bk.tLocType,
        county:bk.tCounty,
        office_name:bk.tOfficeName||'',
        fee:'$'+bk.tFee,
        signers:bk.tSigners,
        signing_address:bk.tAddr||locDesc,
      ...
      fetch('https://formspree.io/f/xqenkybe',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(tbd)}).catch(function(){});
      $('successTitle').textContent='Title Company Booking Confirmed!';
      $('successMsg').textContent='Booked for '+titleFinalTime+'. Pre-negotiated fee: $'+bk.tFee+'. Added to our calendar. Confirmation '+(tIsEmail?'emailed to '+tContact:'sent to '+tContact)+'.';
    ...
    fetch('https://formspree.io/f/xqenkybe',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(resv)}).catch(function(){});
...
// ---- RON info modal (Beth) ----
// Standalone info popup — unlike the booking modal it's safe to dismiss by
// clicking the backdrop or pressing Escape, since there's no form state to lose.
(function(){
  var ov=$('ronOverlay'), md=$('ronModal'), cl=$('ronClose');
  if(!ov) return;
  function openRon(){ov.classList.add('active');document.body.style.overflow='hidden';if(md)md.scrollTop=0;}
  function closeRon(){ov.classList.remove('active');document.body.style.overflow='';}
  // Opened from Beth's card button and from the hero announcement banner.
  ['ronInfoBtn','ronHeroBtn'].forEach(function(id){
    var b=$(id); if(b) b.addEventListener('click',openRon);
  });
  if(cl)cl.addEventListener('click',function(e){e.stopPropagation();closeRon();});
  if(md)md.addEventListener('click',function(e){e.stopPropagation();});
  ov.addEventListener('click',closeRon);
  document.addEventListener('keydown',function(e){
    if((e.key==='Escape'||e.keyCode===27)&&ov.classList.contains('active'))closeRon();
  });
})();
